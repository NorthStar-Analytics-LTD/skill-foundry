...
6. **"What happens to the original after a successful migration?"** (redirect, archive notice, sunset date?) — the skill includes the post-migration step so nothing gets migrated twice or edited in two places. Parallel editing is how mid-migration incidents happen.
...
Include this in the generated `.claude/skills/migrate-chart/SKILL.md`, as its final section:

> **When you hit a source construct with no row in the conversion table** — some LookML refinement, Tableau calculation type or Metabase quirk this file's table does not cover — stop and ask the analyst running the migration how they want it handled, the way you would ask a senior engineer who has seen this tool pair before. Once resolved, do not just finish the one migration: propose a diff to this file adding the new row to the conversion table, with the real example that triggered it, and open it as a PR like any other change to this repo.
>
> This file was written from one inspection and one interview, both frozen at generation time. Every analyst who migrates something afterwards is testing the conversion table against a case the builder never saw. A `/migrate-chart` that never grows its own table is a `/migrate-chart` that will keep asking the same question forever instead of learning the answer once.
